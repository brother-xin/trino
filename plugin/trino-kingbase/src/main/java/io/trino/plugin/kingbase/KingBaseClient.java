        ...
        extends BaseJdbcClient
{
    ...
    private static final int KINGBASE_MAX_SUPPORTED_TIMESTAMP_PRECISION = 6;
    private static final int PRECISION_OF_UNSPECIFIED_DECIMAL = 0;

    private static final DateTimeFormatter TIME_FORMATTER = DateTimeFormatter.ofPattern("HH:mm:ss.SSSSSS");

    private static final PredicatePushdownController KINGBASE_STRING_COLLATION_AWARE_PUSHDOWN = (session, domain) -> {
        if (domain.isOnlyNull()) {
            return FULL_PUSHDOWN.apply(session, domain);
        }

        if (KingBaseSessionProperties.isEnableStringPushdownWithCollate(session)) {
            return FULL_PUSHDOWN.apply(session, domain);
        }
            ...
            RemoteQueryModifier queryModifier)
    {
        ...
        if (kingBaseConfig.isIncludeSystemTables()) {
        ...
        this.aggregateFunctionRewriter = new AggregateFunctionRewriter<>(
                ...
                ImmutableSet.<AggregateFunctionRule<JdbcExpression, String>>builder()
                        ...
                        .add(new ImplementSum(KingBaseClient::toTypeHandle))
    ...
    public List<JdbcColumnHandle> getColumns(ConnectorSession session, JdbcTableHandle tableHandle)
    {
        ...
        try (Connection connection = connectionFactory.openConnection(session)) {
            ...
            if (KingBaseSessionProperties.getArrayMapping(session) == KingBaseConfig.ArrayMapping.AS_ARRAY) {
        ...
        switch (typeHandle.getJdbcType()) {
            ...
            case Types.TIMESTAMP:
                ...
                return Optional.of(ColumnMapping.longMapping(
                        ...
                        KingBaseClient::shortTimestampWriteFunction));
    ...
    private Optional<ColumnMapping> arrayToTrinoType(ConnectorSession session, Connection connection, JdbcTypeHandle typeHandle)
    {
        ...
        KingBaseConfig.ArrayMapping arrayMapping = KingBaseSessionProperties.getArrayMapping(session);
        if (arrayMapping == KingBaseConfig.ArrayMapping.DISABLED) {
            return Optional.empty();
        }
        ...
        if (arrayMapping == KingBaseConfig.ArrayMapping.AS_ARRAY) {
        ...
        if (arrayMapping == KingBaseConfig.ArrayMapping.AS_JSON) {
            return baseElementMapping
    ...
    public WriteMapping toWriteMapping(ConnectorSession session, Type type)
    {
        ...
        if (type instanceof TimeType timeType) {
            if (timeType.getPrecision() <= KINGBASE_MAX_SUPPORTED_TIMESTAMP_PRECISION) {
                return WriteMapping.longMapping(format("time(%s)", timeType.getPrecision()), timeWriteFunction(timeType.getPrecision()));
            }
            return WriteMapping.longMapping(format("time(%s)", KINGBASE_MAX_SUPPORTED_TIMESTAMP_PRECISION), timeWriteFunction(KINGBASE_MAX_SUPPORTED_TIMESTAMP_PRECISION));
        }

        if (type instanceof TimestampType timestampType) {
            if (timestampType.getPrecision() <= KINGBASE_MAX_SUPPORTED_TIMESTAMP_PRECISION) {
                verify(timestampType.getPrecision() <= TimestampType.MAX_SHORT_PRECISION);
                return WriteMapping.longMapping(format("timestamp(%s)", timestampType.getPrecision()), KingBaseClient::shortTimestampWriteFunction);
            }
            verify(timestampType.getPrecision() > TimestampType.MAX_SHORT_PRECISION);
            return WriteMapping.objectMapping(format("timestamp(%s)", KINGBASE_MAX_SUPPORTED_TIMESTAMP_PRECISION), longTimestampWriteFunction());
        }
        if (type instanceof TimestampWithTimeZoneType timestampWithTimeZoneType) {
            if (timestampWithTimeZoneType.getPrecision() <= KINGBASE_MAX_SUPPORTED_TIMESTAMP_PRECISION) {
            ...
            return WriteMapping.objectMapping(format("timestamptz(%d)", KINGBASE_MAX_SUPPORTED_TIMESTAMP_PRECISION), longTimestampWithTimeZoneWriteFunction());
        }
        ...
        if (type instanceof ArrayType arrayType && KingBaseSessionProperties.getArrayMapping(session) == KingBaseConfig.ArrayMapping.AS_ARRAY) {
            Type elementType = arrayType.getElementType();
            String elementDataType = toWriteMapping(session, elementType).getDataType();
            return WriteMapping.objectMapping(elementDataType + "[]", arrayWriteFunction(session, elementType, TypeUtils.getArrayElementPgTypeName(session, this, elementType)));
        }
        ...
        if (isVarchar) {
            ...
            switch (operator) {
                ...
                case GREATER_THAN_OR_EQUAL:
                    return KingBaseSessionProperties.isEnableStringPushdownWithCollate(session);
                case EQUAL:
                case NOT_EQUAL:
    ...
    private static ColumnMapping charColumnMapping(int charLength)
    {
        ...
        return ColumnMapping.sliceMapping(
                ...
                charWriteFunction(),
                KINGBASE_STRING_COLLATION_AWARE_PUSHDOWN);
    }
    ...
    private static ObjectWriteFunction longTimestampWriteFunction()
    {
        return ObjectWriteFunction.of(LongTimestamp.class, ((statement, index, timestamp) -> {
            ...
            verify(KINGBASE_MAX_SUPPORTED_TIMESTAMP_PRECISION == 6);
    ...
    private static ColumnMapping timestampWithTimeZoneColumnMapping(int precision)
    {
        ...
        checkArgument(precision <= KINGBASE_MAX_SUPPORTED_TIMESTAMP_PRECISION, "unsupported precision value %s", precision);
    ...
    private static JdbcTypeHandle getArrayElementTypeHandle(Connection connection, JdbcTypeHandle arrayTypeHandle)
    {
        ...
        try {
            TypeInfo typeInfo = connection.unwrap(KbConnection.class).getTypeInfo();
            int kbElementOid = typeInfo.getKBArrayElement(typeInfo.getKBType(jdbcTypeName));
            verify(arrayTypeHandle.getCaseSensitivity().isEmpty(), "Case sensitivity not supported");
            return new JdbcTypeHandle(
                    typeInfo.getSQLType(kbElementOid),
                    Optional.of(typeInfo.getKBType(kbElementOid)),
